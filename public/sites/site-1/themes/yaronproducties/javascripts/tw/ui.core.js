...
var _remove = JQ.fn.remove,
	isFF2 = JQ.browser.mozilla && (parseFloat(JQ.browser.version) < 1.9);
...
if (isFF2) {
	var attr = JQ.attr,
		removeAttr = JQ.fn.removeAttr,
		ariaNS = "http://www.w3.org/2005/07/aaa",
		ariaState = /^aria-/,
		ariaRole = /^wairole:/;

	JQ.attr = function(elem, name, value) {
	...
	JQ.fn.removeAttr = function(name) {
...
function getter(namespace, plugin, method, args) {
	function getMethods(type) {
		var methods = JQ[namespace][plugin][type] || [];
...
JQ.widget = function(name, prototype) {
	...
	JQ.fn[name] = function(options) {
		...
		return this.each(function() {
			...
			(!instance && !isMethodCall &&
				JQ.data(this, name, new JQ[namespace][name](this, options))._init());

			// method call
			(instance && isMethodCall && JQ.isFunction(instance[options]) &&
	...
	JQ[namespace][name] = function(element, options) {
		...
		this.widgetEventPrefix = JQ[namespace][name].eventPrefix || name;
		this.widgetBaseClass = namespace + '-' + name;

		this.options = JQ.extend({},
			JQ.widget.defaults,
			JQ[namespace][name].defaults,
			JQ.metadata && JQ.metadata.get(element)[name],
			options);
	...
	JQ[namespace][name].getterSetter = 'option';
};
...
JQ.widget.prototype = {
	...
	_trigger: function(type, event, data) {
		...
		return !(JQ.isFunction(callback) && callback.call(this.element[0], event, data) === false
...
JQ.ui.mouse = {
	...
	_mouseDown: function(event) {
		...
		var self = this,
			...
			elIsCancel = (typeof this.options.cancel == "string" ? JQ(event.target).parents().add(event.target).filter(this.options.cancel).length : false);
		...
		(JQ.browser.safari || event.preventDefault());
	...
	_mouseMove: function(event) {
		...
		if (JQ.browser.msie && !event.button) {
