class Admin::ThemesController < Admin::BaseController
  ...
  def import
    render :layout => false and return unless request.post? # renders the import form

    begin
      file = params[:theme] ? params[:theme][:file] : nil
      if file.blank?
        render :text => t(:'adva.themes.flash.import.error_filename_blank') and return
      elsif @site.themes.import(file)
        render :text => "OK" and return
      else
        render :text => t(:'adva.themes.flash.import.failure') and return
      end
    rescue
      render :text => t(:'adva.themes.flash.import.failure') and return
    end
